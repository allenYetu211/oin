	...
	return (
		<>
			...
			<div className="h-[40vh] p-2 flex  flex-col space-y-2 rounded-t-lg shadow-lg">
				<Button color="primary" variant="bordered" onClick={currentRecodingState}>
				...
				<Modal isOpen={isOpen} placement={'auto'} size="5xl" onOpenChange={onOpenChange}>
					<ModalContent className="h-[80vh]">
						{(onClose) => (
							<>
								<ModalHeader className="flex flex-col gap-1">Recording History</ModalHeader>
								<ModalBody className="overflow-auto">
									{!!walkHistory.length && (
										...
										>
											{walkHistory.map((item: WalkHistoryProps, index: number) => (
												<ListboxItem key={index}>
													<div className="flex justify-between items-center">
														<div>{item.time && dayjs(item.time).format('YYYY-MM-DD HH:mm:ss')}</div>
														<div>Timer: {convertToTimeFormat(item.timer)}</div>
													</div>
												</ListboxItem>
											))}
										</Listbox>
